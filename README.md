...
- 楽譜は最初の検出音を1拍目とする4/4・16分単位の推定です。調・拍子の自動推定はなく、テンポ候補にも倍/半分などの曖昧さがあります。量子化で消えた短音は件数を表示し、譜面再生も同じ省略を反映します。PDF/MIDI出力は含みません。鼻歌の音符化・同音再発音・再生との一致の[比較と限界](docs/evaluation/melody-verification.md)を参照してください。
