...
倍音の誤選択と短い音の分断を調整しています。マイクのノイズ抑制は無効を要求し、実適用値を画面に表示します。静かな場所で録音してください。前後の安定した声へ20ms以内に戻る大きな急落は、表示・ピアノ・標準採譜で共通に限定補正します。元音声と内部の原解析値は保持します。ピアノには孤立音・境界倍音の補正もあります。[倍音の比較](docs/evaluation/pitch-continuity.md)・[急落補正の条件と限界](docs/evaluation/brief-pitch-drops.md)を参照してください。

## 操作方法

1. 「歌声」か「話し声」を選び、「録音する」でマイクを許可します。
2. 最初の約0.3秒は静かにし、そのあと声を出してください。明瞭な声は開始直後でも解析します。最大60秒で自動停止します。
3. 「録音を止める」のあと「分析中」の進捗を待ちます。前後の音を見直したら、「元の声 / ピアノ」を選び、ひとつの再生ボタンで聴きます。分析だけ失敗した場合は元音声を残し、再生設定から再解析できます。
4. 歌声・話し声ともに「ピアノ（鍵盤の音程）」が初期設定です。一音ずつ固定した鍵盤の音程で鳴り、打鍵後に自然に減衰します。声のしゃくりや細かな揺れを聴く場合は、再生設定で「声の揺れを聴く（連続音高）」を選べます。
...
7. 「譜面どおりに聴く」は、表示と同じ音符・休符・タイで、自然に減衰するピアノを鳴らします。離鍵後に短い余韻があります。テンポ候補（根拠が弱い場合は仮120）を40〜240 BPMで調整すると、譜面とこの再生が一緒に変わります。元の声や上のピアノ再生は変わりません。新録音・別の再生・パネルを閉じる操作で譜面再生は止まります。
...
10. 五線譜の音符または「音符を直す」の一覧から、音程・開始・長さを最短1/32拍（128分音符）単位で変更できます。休符化・音符追加・50件のUndo/Redoに対応します。手直し後のテンポ変更は音価を保って速度だけを変えます。推定の切替では各推定の手直しを保持します。再解析・ファイル読込の成功や録音開始で手直しは消えますが、再解析・読込の失敗時は直前の結果を保持します。
11. 「MIDIを保存」で、表示・譜面再生と同じ音程・音価・テンポを楽器アプリへ渡せます。MIDIはカタカナや五線譜の画像、元の音声を含みません。PDF出力は未対応です。

青い線が短い急落を補正した推定の声の高さ、薄いバーが近くの半音です。「正解メロディ」や採点ではありません。無音・無声音・不明な区間は空白です。録音中は後続の支持音が揃ってから軌跡を補正するため、表示は暫定です。同形の20ms以下の本物の低音との完全な識別はできません。
...
- 一人の近くの声、静かな場所を基本条件とします。55〜1000 Hz。YIN、歌80ms / 話60msの窓、10ms更新です。マイク録音の冒頭300msの非周期音で環境音を測定し、有声判定できた即発声は残します。
- 初期雑音後の長音欠落を修正し、回復値が弱い別音源へ引き継がれる回帰にも対応しました。回復値を発声区間へ結び付け、実測した候補と音量から次の音を判定します。周期環境音や弱い別音高の開始には識別限界があります。[修正前後・回帰検証・実機未確認事項](docs/evaluation/source-scoped-recovery.md)を参照してください。
...
- 楽譜は最初の検出音を1拍目とする4/4の推定です。検出済みの短音・別発音・休符を、各境界50ms以内の調整で保持し、最短128分音符まで記譜します。調・拍子の自動推定はなく、テンポ候補にも倍/半分などの曖昧さがあります。表現できない原音符は対象・理由とともに保持し、未配置や要確認として案内します。[短音保持の設計と比較](docs/evaluation/score-preservation.md)を参照してください。鼻歌の音符化・同音再発音・再生との一致の[比較と限界](docs/evaluation/melody-verification.md)を参照してください。
