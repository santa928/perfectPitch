...
- 初期雑音後の長音欠落を改善中です。校正値の回復と録音後の再判定により、定常雑音付きの指定入力は末尾まで保持します。ただし、先行する声の後の周期背景音を拾う回帰が未解決で、PRはDraftです。[追加レビューの数値・失敗テスト・未確認事項](docs/evaluation/reviewed-sustained-noise.md)を参照してください。
