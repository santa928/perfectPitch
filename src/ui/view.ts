...
export function mountView(root: HTMLElement) {
  ...
  <main class="app">
    ...
    <section class="studio" aria-label="声の録音と聴き比べ">
      ...
      <div class="transport"><div class="capture-actions"><button id="record" class="record-button"><span aria-hidden="true">●</span> 録音する</button><button id="importAudio" class="text-button" type="button" aria-describedby="importHelp">音声ファイルを開く</button><button id="cancelImport" class="text-button" type="button" hidden>読み込みを中止</button><input id="audioFile" type="file" accept="audio/*,.m4a,.wav,.mp3,.ogg,.flac,.aac,.webm" hidden></div><p id="importHelp">ボイスメモなど、60秒・20MBまで。対応形式はブラウザにより異なります。</p><p id="status" role="status" aria-live="polite">歌声を選んでいます。最大60秒まで録音できます。</p></div>
      <section id="review" class="review" aria-label="録音の聴き比べ" hidden>
        ...
        <details class="settings"><summary>再生・解析の設定</summary><div class="settings-body"><label>ピアノの鳴らし方<select id="pitchMode"><option value="rounded">ピアノ（鍵盤の音程）</option><option value="continuous">声の揺れを聴く（連続音高）</option></select></label><p id="pitchHelp">一音ずつピアノを打鍵し、自然に減衰します。</p><button id="reanalyze" class="text-button">元音声から再解析する</button><p id="pianoStatus">ピアノは選択後の再生時に音源を読み込みます。</p></div></details>
      </section>
      <section id="score" aria-label="録音から作る推定の楽譜" hidden></section>
    </section>
    <section class="notes"><div><h2>声から生まれる、小さなメロディ。</h2><p>鼻歌は「楽譜とドレミを見る」から、音符にまとめたメロディを聴き、手直ししてMIDIを保存できます。声の細かな揺れを聴くときは、再生設定で「声の揺れを聴く（連続音高）」を選んでください。</p></div><details><summary>録音について・使える環境</summary><p>一人の近くの声を、静かな場所で。息や雑音、判定できない区間は軌跡の空白になります。音程表示の対応音域は55〜1000 Hzです。伴奏や他の人の声の分離はできません。</p><p>音声はブラウザ内で処理し、サーバーへ送信しません。音声と手直しはこの画面を閉じると消えるため、必要な楽譜はMIDIを保存してください。ピアノ音源の取得時は外部サイトへ接続します。任意の再採譜モデルはこのサイトから読み込みます。</p><p>録音はHTTPSとAudioWorklet、ファイル入力はWeb Audioとブラウザの対応デコーダーが必要です。画面を離れた場合は録音・再生を停止します。iPhone／Androidの実機評価は未完了です。</p><p id="micSettings">マイクの適用設定は録音後に表示します。</p></details></section>
  ...
  return {
    ...
    importAudio: get<HTMLButtonElement>('importAudio'),
    audioFile: get<HTMLInputElement>('audioFile'),
    cancelImport: get<HTMLButtonElement>('cancelImport'),
