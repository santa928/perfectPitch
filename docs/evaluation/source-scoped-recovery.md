...
校正無効のファイル解析で、先頭0.4秒以上の無音によりfloorが0.0003へ下がり、peak0.0015の微小周期音を音符化する退行を実モジュールで再現した。初期値0.001の下限を高速再測定と緩やかな追従の両方で維持するよう修正。0.4/3秒の無音導入、歌/話、live/offlineで有声0・音符0を確認する単体テストと、実WAVファイル入力でも音符化せず原音を保持するブラウザテストを追加した。マイクの校正有効時は変更しない。
